...
SOURCE_PDF = Path(
    ...
    "ISOMETRICS FOR PTFE SPOOL PREPARATION-1044-1150-71.pdf"
)
SOURCE_PAGE_INDEX = 0
OUTPUT = ROOT / "outputs"
OUTPUT.mkdir(exist_ok=True)
PDF_PATH = OUTPUT / "ISO_71_PTFE.pdf"
DXF_PATH = OUTPUT / "ISO_71_PTFE.dxf"
DWG_PATH = OUTPUT / "ISO_71_PTFE.dwg"
PNG_PATH = OUTPUT / "ISO_71_PTFE.png"
...
def build_entities():
    ...
    add_text(292, 35.2, '1"-SNA-424-1501-A82Y-D', 2.9, "HIGHLIGHT", bold=True)
    ...
    bom_rows = [
        ...
        ["NO", "", "(INS)", "", ""],
        ["1", 'PIPE SPOOL (1492 Lg)', '1"', "PI3D217Z00613ZZZZ", "1 NO"],
        ["2", 'PIPE SPOOL (100 Lg)', '3/4"', "PI3D277Z00514ZZZZ", "1 NO"],
        ["3", 'PIPE SPOOL (1677 Lg)', '1"', "PI3D217Z00613ZZZZ", "1 NO"],
        ["4", 'PIPE SPOOL (465 Lg)', '1"', "PI3D217Z00613ZZZZ", "1 NO"],
        ["5", 'PIPE SPOOL (3104 Lg)', '1"', "PI3D217Z00613ZZZZ", "1 NO"],
        ["6", 'T.RED B-16.9 BW 40S 80S', '1 x 3/4', "WRGC84ZZ006130514", "1 NO"],
        ["7", 'ELBOW.45 B-16.9 BW 1.5D 40S', '1"', "WBGC84Z100613ZZZZ", "2 NO"],
        ["8", 'FLNG.WN B-16.5 150 RF/125AARH', '3/4"', "FWCG527Z00514ZZZZ", "1 NO"],
        ["9", 'FLNG.BLIND B-16.5 150 RF/125AARH', '3/4"', "FBCG527Z005ZZZZZ", "1 NO"],
        ["10", 'VLV.BALL SHEET 543DR', '3/4"', "543DRZZZ005ZZZZZ", "1 NO"],
    ]
    add_table(15.0, 85.0, bom_w, 6.0, bom_rows, font_size=2.2)

    # -------------------------------------------------------------
    # PIPE MODEL TOPOLOGY (PAGE 71)
    # -------------------------------------------------------------

    # Start Continuation Node (Bottom-Left)
    p0 = (85.0, 180.0)
    add_text(p0[0] - 5, p0[1] - 15, "CONT. ON 1\"-SNA-424-1501-1-A82Y-A\nE 676662 N 543456 EL +106217",
             2.25, "LABEL", bold=True)

    # Leg 1: Segment <5> (30° Isometric Axis)
    u30 = (math.cos(math.radians(30.0)), math.sin(math.radians(30.0)))
    p1 = (p0[0] + 110.0 * u30[0], p0[1] + 110.0 * u30[1]) # (180.2, 235.0)
    ...
    # 45° Elbow at p1
    add_small_callout("7", (p1[0] - 5.0, p1[1] + 12.0), p1)

    # Leg 2: 45° Offset Loop Segment <4> (Exact 45.0° angle: dx = 35, dy = 35)
    p2 = (p1[0] + 35.0, p1[1] + 35.0)
    draw_pipe(p1, p2)
    ...
    add_text((p1[0] + p2[0]) / 2, (p1[1] + p2[1]) / 2 + 4.0, "<4> [1]", 2.35, "LABEL", bold=True, align="center")
    add_rect(p2[0] + 4.0, p2[1] - 8.0, 7.0, 5.0, "DIM", 0.20)
    add_text(p2[0] + 7.5, p2[1] - 7.0, "45°", 2.0, "DIM", bold=True, align="center")

    # 45° Elbow at p2
    add_small_callout("7", (p2[0] + 10.0, p2[1] - 12.0), p2)

    # Leg 3: Segment <3> (30° Isometric Axis)
    ...
    # Leg 4: Main Run Segment <1> (30° Isometric Axis)
    ...
    # Vertical Branch Leg at p3 (90° Vertical Axis)
    add_text(p3[0] + 5, p3[1] - 5, "1\"x3/4\"NS", 2.25, "LABEL", bold=True)
    ...
    # Flange 8 at branch top
    add_flange(branch_top[0], branch_top[1], 90.0)
    add_small_callout("8", (branch_top[0] + 15.0, branch_top[1]), branch_top)

    # Ball Valve 10
    valve_c = (branch_top[0], branch_top[1] + 10.0)
    add_valve_ball(valve_c, 90.0)
    add_small_callout("10", (valve_c[0] + 15.0, valve_c[1]), valve_c)

    # Blind Flange 9 at top
    blind_top = (branch_top[0], branch_top[1] + 20.0)
    add_flange(blind_top[0], blind_top[1], 90.0)
    add_small_callout("9", (blind_top[0] + 15.0, blind_top[1]), blind_top)

    add_single_dimension(branch_top, blind_top, "118", offset=10.0, oblique_angle=30.0)
...
def export_dxf():
    ...
    for name, (color, lineweight) in layer_specs.items():
        if name not in doc.layers:
            doc.layers.add(name, color=color, lineweight=lineweight, linetype="Continuous" if name != "CENTER" else "CENTER")
...
def export_pdf_and_png():
    ...
    scr_path = OUTPUT / "conv_iso71.scr"
    scr_path.write_text(
